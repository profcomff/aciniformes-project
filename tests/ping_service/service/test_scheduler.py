...
@pytest.mark.asyncio
async def test_add_fetcher_success(pg_scheduler_service, fetcher_obj):
...
@pytest.mark.asyncio
async def test_delete_fetcher(pg_scheduler_service, fetcher_obj):
    pg_scheduler_service.add_fetcher(fetcher_obj)
    fetchers = pg_scheduler_service.get_jobs()
...
@pytest.mark.asyncio
async def test_get_jobs(pg_scheduler_service):
    res = pg_scheduler_service.get_jobs()
    assert type(res) is list


@pytest.mark.asyncio
async def test_start_already_started(pg_scheduler_service, crud_client):
...
@pytest.mark.asyncio
async def test_ping_fail(pg_scheduler_service, fetcher_obj, dbsession):
    fetcher = Fetcher(
        **{
            "type_": "ping",
            "address": "fasdlj",
